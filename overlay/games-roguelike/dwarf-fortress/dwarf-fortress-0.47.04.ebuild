...
# Based on games-roguelike/dwarf-fortess-0.44.12 from overlay gentoo
...
src_prepare() {
	rm -f libs/libstdc++.so* || die
	sed -i -e '1i#include <cmath>' g_src/ttf_manager.cpp || die
	sed -i -e 's/ALenum alGetError( void ) { _alGetError(); }/ALenum alGetError( void ) { return _alGetError(); }/' g_src/music_and_sound_openal.cpp || die
	default
}
